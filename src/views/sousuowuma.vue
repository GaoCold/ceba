<template>
  <div style="margin-left:232px;padding-top:30px;position:relative">
    ...
    <div style="width:100%;height:650px;background:#fff;position:relative;">
      <ul class="qiye_ul">
        <li>序号</li>
        <li>企业名称</li>
      ...
      <ul class="qiye_ul" v-for="(qy,i) in qiyeList" :key="i">
        <li>{{i+1 +14*num}}</li>
        <li>
          <router-link :to="{name:'xiangqing',params:{name:qy.orgName}}">{{qy.orgName}}</router-link>
        </li>
        <li>{{qy.orgCode}}</li>
      </ul>
...
export default {
  ...
  data() {
    return {
      ...
      zhonglei: this.$route.params.zhonglei
    };
  },
  mounted() {
    ///要改
    ...
    this.getfuzzyQuery({
      ...
      keyWord: this.$route.params.sousuo,
      searchType: this.$route.params.zhonglei,
  ...
  methods: {
    ...
    goback() {
      console.log(666);
      this.$router.push("/home/homeye/:" + 123);
      //   this.$router.push({
      //     name: "homeye",
      //     params: {
      //       change_id: "000" + new Date()
      //     }
      //   });
    },
    gotid() {
    ...
    getqiye2() {
      console.log("6666");
      if (this.id == 0) {
        if (typeof this.sousuovalue == string) {
          alert("6666666666");
        }
      }
      this.getfuzzyQuery({
